...
CREATE TABLE `Author` (
  ...
  UNIQUE index uq_authorName (`authorName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `AuthorPublicationMap` (
  ...
  CONSTRAINT `authorpublicationmap_ibfk_1` FOREIGN KEY (`publicationId`) REFERENCES `Publication` (`publicationId`),
  CONSTRAINT `authorpublicationmap_ibfk_2` FOREIGN KEY (`authorId`) REFERENCES `Author` (`authorId`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `BookChapterData` (
  ...
  UNIQUE index uq_bookChapterName (`bookChapterName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `Conference` (
  ...
  PRIMARY KEY (`conferenceId`),
    UNIQUE index uq_conferenceName (`conferenceName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `Journal` (
  ...
  PRIMARY KEY (`journalId`),
    UNIQUE index uq_journalName (`journalName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `School` (
  ...
  PRIMARY KEY (`schoolId`),
   UNIQUE index uq_schoolName (`schoolName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
